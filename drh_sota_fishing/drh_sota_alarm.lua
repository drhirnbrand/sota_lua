...
updateMinimumInMs = 100; -- the minimum number of ms between each update
updateDriftSpeed = 0.025; -- drift of health/focus per time, how fast the bar moves to the current value after a change
...
INDEX_MAIN = { panel = 1 };
INDEX_TITLE = { panel = 1, text = 5 };
INDEX_PLAYER_HEALTH = { panel = 2, bar = 2, down = 3, up = 4, text = 2, tx_bg = 1, tx = 2, tx_up = 3, tx_down = 4 };
INDEX_PLAYER_FOCUS = { panel = 3, bar = 5, down = 6, up = 7, text = 3, tx_bg = 1, tx = 5, tx_up = 6, tx_down = 7 };
INDEX_SUMMON_HEALTH = { panel = 4, bar = 8, down = 9, up = 10, text = 4, tx_bg = 1, tx = 8, tx_up = 9, tx_down = 10 };
...
playerHealthPrevious = 0;
playerFocusPrevious = 0;
summonHealthPrevious = 0;

LogPrefixInfo = "[0000ff]drh_sota_healthbar: "
LogPrefixWarn = "[ff0000]drh_sota_healthbar: "
LogPrefixDebug = "[ff0000]DEBUG drh_sota_healthbar: "
LogSuffix = "[-]"

--- This is a structure that stores the main information objects
---
HEALTHBAR = HEALTHBAR or {
    ...
    textsSpec = {
        ...
        { fontSize = 10, text = "0 / 0", color = "#ffffff" },
        { fontSize = 10, text = "0 / 0", color = "#ffffff" },
        { fontSize = 10, text = "DRH Healthbar " .. VersionText, color = "#8f8f8f" }
    };
...
function ShroudOnStart()
    ShroudConsoleLog(string.format(LogPrefixInfo .. "Health Monitor %s" .. LogSuffix, VersionText));
...
function ShroudOnUpdate()
    ...
    if (now - timestamp) < updateMinimumInMs then
        return
    end
    timestamp = now;
    ...
    delta = ShroudDeltaTime * playerMaxHealth * updateDriftSpeed;

    playerHealthPrevious = drift(playerHealth, playerHealthPrevious, delta);
    playerFocusPrevious = drift(playerFocus, playerFocusPrevious, delta);
    summonHealthPrevious = drift(summonHealth, summonHealthPrevious, delta);

end

function drift(current, previous, delta)
    local difference = current - previous;
    local drift = sign(difference) * (delta * updateMinimumInMs);
...
function updateAndSavePositions()
    ...
    mainPanel.x = mainPosition.x;
    mainPanel.y = mainPosition.y;
